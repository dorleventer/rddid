...
| `rd_compstable()` | the recipe's "above-cutoff units" are the original **below**-cutoff units; the literal mirrored jump is $\tilde\pi_{t_\mathrm{RD},(+)}(1)-\tilde\pi_{t_0,(+)}(1)=\pi_{t_\mathrm{RD},(-)}(0)-\pi_{t_0,(-)}(0)$ | **convention (Dor, 2026-09-10): type keeps the original orientation**, so the reported jump is $\pi_{t_\mathrm{RD},(-)}(1)-\pi_{t_0,(-)}(1)$ = minus the literal mirrored jump; Wald identical ($\pi(0)=1-\pi(1)$); the figure then reads off the type-continuity panel | **Yes** |
...
- **2026-09-10, code (rddid `745c5ed`).** Five functions gained `estimand`; `rd_compstable()` mirrors; ties error. Full suite `[ FAIL 0 | WARN 2 | SKIP 0 | PASS 2224 ]` (the 2 warnings are the pre-existing `rd_bw_cct` fallback test; 2224 is the post-`cfd0c3c` baseline, the permutation/McCrary tests having been removed). Lead smoke script: all PASS (rddid atu == att; mirrored == −att with equal SE; typecont/homog stats equal; compstable atu == att-on-mirrored to 1e-12 and == independent below-side fit; tie error fires).
- **2026-09-10, invariance tests (`dc1eeec`).** `test-mirror-invariance.R`: 210 expectations PASS at 1e-10 (bandwidths 1e-8) on DGP-A (PC, 3 bandwidth rules, linear weights) and DGP-B S1 (PV); no rdrobust asymmetry found.
- **2026-09-10, API tests (`e55aa78`).** `test-estimand.R`: 60 expectations PASS (validation, label-only equality, compstable atu == att-on-mirrored, from-the-equations below-side fit, tie error, prints).
- **2026-09-10, manual Grembi checks 7–8.** `s6_estimates.R` with `estimand = "atu"` on every `rddid()` call: `s6_main.tex`, `s6_numbers.tex` byte-identical apart from the generated-by header. `s5_application.R` with `estimand = "atu"` on every test call: A7/A9/A10 macros and the validation table's other rows byte-identical; **A8 (composition stability, below-cutoff shares) new numbers**: jump $+0.37$ (SE $0.11$, $p=0.001$), BC $+0.36$ (SE $0.14$, $p=0.011$), $\chi^2(1)=10.67$ conv / $6.51$ BC — was (above-cutoff, ATT version) $-0.25$ (SE $0.09$, $p=0.008$), BC $p=0.030$. Package jump equals the hand-built below-side reflected fit ($0.374$; `stopifnot` in the script). Reading: the share of below-cutoff municipalities that are below the cutoff in the other census is $0.37$ higher in 2004 than in 2000 — the same unbalanced upward crossings, seen from below; composition stability still rejected, more strongly.
- **2026-09-10, site (`b5640e7`, pushed).** Get-started closing section + validation "ATU designs" section, README line, article title, site-plan note. `R CMD check --as-cran` with vignettes: 0 errors, 0 warnings, 1 NOTE (clock). CI green (pkgdown 2m17s, R-CMD-check 3m18s); live site verified (both articles and `rd_compstable` reference carry `estimand`). Installed package = repo HEAD. **Paper side (§6 step 6) pending Dor's approval of the §7.3 prose**; the figure panel (b) is already rebuilt on the below-cutoff units (`s5_validation_figure.R`, gap 0.374) and `s5_numbers.tex`/`s5_validation.tex` carry the new A8 macros.
- **2026-09-10, convention change (Dor).** Under `estimand = "atu"` the TYPE keeps its original orientation ("above the cutoff in the other period"); only the sample moves to the below-cutoff units. Reported jump $=\pi_{t_\mathrm{RD},(-)}(1)-\pi_{t_0,(-)}(1)$ (= minus the literal mirrored-recipe jump; Wald identical). Reason: pure labeling ($\pi(0)=1-\pi(1)$), and this way the paper's figure panel (b) is literally the two below-cutoff halves of the type-continuity panel (2000 as is, 2004 mirrored). Tests updated (`test-estimand.R`: jumps negated vs att-on-mirrored; hand-built fit with the "above" indicator); suite 2494/0; check clean. Grembi A8 becomes jump $-0.37$ (same SE, $p$, $\chi^2$).
- **2026-09-10, Grembi after the convention change (rddid `a7e34cf` installed).** `s5_application.R` rerun: A8 jump $-0.37$ (SE $0.11$, $p=0.001$), BC $-0.36$ (SE $0.14$, $p=0.011$), $\chi^2(1)=10.67/6.51$ unchanged; the script's hand-built below-side fit (type = above in the other census) reproduces the package jump ($-0.374$, `stopifnot`). Figure panel (b) redrawn with the same indicator (2000 as is, 2004 mirrored; gap $-0.374$). Everything else byte-identical.
