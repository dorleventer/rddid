...
**Last synced.** Paper: `rd-did` commit `4ce8526` (2026-09-08; App. B.3–B.4 rewrite `795ca4a`..`e187613`, then the App. B review fixes `b3ae549`, pilot-bandwidth rates `bdd08e8`, §5.3 sync `6eae87d`). Symbols: the paper's kernel constants are $\nu_{(\blacktriangle),p}$ (bias), $\omega_{(\blacktriangle),p}$ (variance), $\omega_{(\blacktriangle),p}(\rho)$ (cross-period, $\omega(1)=\omega$); the package's mnemonics are `.kc_b`, `.kc_v`, `.kc_c`.
...
| Residual for $\widehat{\boldsymbol\Sigma}$ | $\widehat\varepsilon_{i,t}$ = own-side $p$-fit at $h_t$ (preamble) for the conventional variance; own-side $q$-fit at $b_t$ for the BC variance and the curvature-variance (B.2 "Estimation", stated since rd-did `4ce8526`, 2026-09-08) | conventional: own-side $p$-fit at $h_t$; BC and `g_diff`: own-side $q$-fit at $b_t$ (`rdrobust` convention) | D1/D4 settled (Dor, 2026-09-08) |
| Finite-sample factor | HC1 factor stated in B.2 "Estimation" (rd-did `4ce8526`) | HC1: residual $\times\sqrt{n_{\blacktriangle}/(n_{\blacktriangle}-(p+1))}$ (conv) or $\sqrt{n_{\blacktriangle}/(n_{\blacktriangle}-(q+1))}$ (BC), $n_{\blacktriangle}$ = side-$\blacktriangle$ units in the pilot window (`rdrobust` `vce="hc1"`, matched to 1e-10 in `test-rd_period.R`) | variances $\approx+0.5$–$0.7\%$ (Study 5c of the App. B simulation verification); D2 settled (Dor, 2026-09-08) |
...
| $\mathcal B(t_{\mathrm{RD}})$, $\mathcal V^{\mathrm S}(t_{\mathrm{RD}})$ (P3) | $\sum_\tau\tilde w_\tau\mathcal B_\tau$; $\sum_\tau\tilde w_\tau^2\mathcal V_\tau/\pi_\tau+\mathbf 1\{\mathrm S=\mathrm{PC}\}\sum_\tau\sum_{s\ne\tau}\tilde w_\tau\tilde w_s\mathcal C_{\tau,s}$ | `.bw_joint()` on `.bw_constants()`: `B <- sum(cf * bt)` $=\sum_\tau$`coef`$_\tau\widehat{\mathcal B}_\tau$ with each $\widehat{\mathcal B}_\tau$ = `rd_period`'s `b_const` at period $\tau$'s **own** CCT pilot; `Veff <- sum(cf^2 * vt / nt)` $=\sum_\tau$`coef`$_\tau^2\widehat{\mathcal V}_\tau/n_\tau\approx\mathcal V^{\mathrm S}/n$, plus under PC the same-side term at $\rho=1$, `2 sum_{t<s} cf_t cf_s (kappa_+ c_+(p,1) + kappa_- c_-(p,1))` from the pilot covariances (same $\kappa$ as `iter`) | **before 2026-09-10 every period was fitted at the RD period's pilot $(h_0,b_0)$**, so the selector depended on the `t_rd` label (arbitrary for an aggregate over several RD periods); `B` was also hard-coded to $p=1$ (fixed 2026-09-08) | MISMATCH → fixed (per-period pilots, 2026-09-10) | `B.4 common h`, `B.4 common h — h* is the scalar minimizer`, `B.4 joint selectors do not depend on ... t_rd` |
...
| Pilot bandwidth $b_t$ for the aggregate | B.4 after `eq:common_h_opt` (since `4ce8526`): $b^\star=h^{\star,\mathrm S}b^{\mathrm{CCT}}_{t_{\mathrm{RD}}}/h^{\mathrm{CCT}}_{t_{\mathrm{RD}}}$; $b_t=h_t\,b^{\mathrm{CCT}}_t/h^{\mathrm{CCT}}_t$ | `.bw_joint`: $b_t=h^\star\cdot b^{\mathrm{CCT}}_t/h^{\mathrm{CCT}}_t$ **per period** (since 2026-09-10; was the RD period's ratio for all periods); `.bw_joint_iter`: $b_t=h_t\cdot b^{\mathrm{CCT}}_t/h^{\mathrm{CCT}}_t$ per period | paper's `joint` sentence still anchors on $t_{\mathrm{RD}}$: **paper-side edit pending** (replace by the per-period ratio, as already written for the period-specific rule) | PACKAGE AHEAD OF PAPER | — |
...
- **D1 (T1474) — SETTLED (Dor, 2026-09-08): paper adopts the `rdrobust` convention.** B.2 "Estimation" states
  that the BC variance uses the residuals of the order-$q$ pilot fit at $b_t$ (rd-did `4ce8526`). Both variants are consistent
  (Study 5 of the App. B verification: `BC/p_h` and `BC/q_b` ratios → 1; gap 0.1–0.9% in the conformance tests).
- **D2 — SETTLED (Dor, 2026-09-08): HC1 factor stated in B.2 "Estimation" (rd-did `4ce8526`).** The package applies `rdrobust`'s
...
- **G1 — CLOSED (rd-did `4ce8526`, 2026-09-08).** Assumption `ass:reg`(f) imposes $b_t\to0$, $n_tb_t^{2p+3}\to\infty$ and
  $h_t/b_t\to\bar\rho_t\in[0,\infty)$ (`bdd08e8`), and B.4 now states the package's choice after `eq:common_h_opt`:
  the pilot keeps the CCT ratio, $b^\star=h^{\star,\mathrm S}b^{\mathrm{CCT}}_{t_{\mathrm{RD}}}/h^{\mathrm{CCT}}_{t_{\mathrm{RD}}}$ (`joint`) and
  $b_t=h_t\,b^{\mathrm{CCT}}_t/h^{\mathrm{CCT}}_t$ (`iter`).
- **G1 follow-up (2026-09-10, session 2026-09-10-03).** Reviewing an aggregate-over-RD-periods estimator (rd-did `drafts/agg_att_note.tex`) showed the common-$h^\star$ plug-in depended on which RD period carried the `t_rd` label, because every period was fitted at that period's CCT pilot. Fixed in the package: per-period pilots for both joint rules (`.bw_constants()`), common $h^\star$ = scalar minimizer of the iterative objective, $b_t=h^\star b^{\mathrm{CCT}}_t/h^{\mathrm{CCT}}_t$ per period. The paper's B.4 sentence ($b^\star=h^{\star,\mathrm S}b^{\mathrm{CCT}}_{t_{\mathrm{RD}}}/h^{\mathrm{CCT}}_{t_{\mathrm{RD}}}$) is now behind the package: Dor to replace it by the per-period form. Table 1's common-$h^\star$ rows move when `s6_estimates.R` is rerun.
